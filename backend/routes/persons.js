...
router.get('/', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT p.*, l.city, l.address as location_address FROM Person p LEFT JOIN Location l ON p.location_id=l.location_id ORDER BY p.name');
    res.json(rows);
...
router.get('/:id', async (req, res) => {
  try {
    const [[row]] = await pool.query('SELECT p.*, l.city, l.address as location_address FROM Person p LEFT JOIN Location l ON p.location_id=l.location_id WHERE p.person_id=?', [req.params.id]);
...
router.post('/', async (req, res) => {
  try {
    const { name, age, gender, phone_number, location_id } = req.body;
    const [result] = await pool.query(
      'INSERT INTO Person (name, age, gender, phone_number, location_id) VALUES (?,?,?,?,?)',
      [name, age, gender, phone_number, location_id || null]
    );
...
router.put('/:id', async (req, res) => {
  try {
    const { name, age, gender, phone_number, location_id } = req.body;
    await pool.query('UPDATE Person SET name=?, age=?, gender=?, phone_number=?, location_id=? WHERE person_id=?',
      [name, age, gender, phone_number, location_id || null, req.params.id]);
